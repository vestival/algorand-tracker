...
### Fixed
- Wallet verification now accepts a valid signed challenge transaction as cryptographic ownership proof even when broadcast/indexer confirmation is transiently unavailable, preventing third-wallet linking failures that surfaced as `Signed transaction could not be submitted or found on-chain yet`; unsigned signed-payload wrappers are still rejected (`src/app/api/wallets/verify/route.ts`, `src/lib/verification/signed-proof.ts`, `tests/signed-proof.test.ts`) (2026-03-03 21:40 UTC)
